...
export const actionAdapterBindings = mysqlTable('action_adapter_bindings', {
  ...
  resolutionContractJson: json('resolution_contract_json').$type<Record<string, unknown>>(),
  resolutionContractHash: char('resolution_contract_hash', { length: 64 }),
  verificationPolicyKey: varchar('verification_policy_key', { length: 160 }),
  verificationPolicyRevision: varchar('verification_policy_revision', { length: 32 }),
  verificationPolicyHash: char('verification_policy_hash', { length: 64 }),
  verificationContractJson: json('verification_contract_json').$type<Record<string, unknown>>(),
  verificationContractHash: char('verification_contract_hash', { length: 64 }),
