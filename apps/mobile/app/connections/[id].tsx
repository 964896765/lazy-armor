...
import { capabilityDescription, capabilityGrantLabel, capabilityHealthLabel, capabilityImplementationLabel, capabilityLabel, capabilityOfficialLabel, capabilityRiskHint, connectionStatusLabel } from '../../src/connection-presenter';
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content} refreshControl={<RefreshControl tintColor={colors.primary} refreshing={connections.isFetching || permissions.isFetching || plans.isFetching || capabilityReality.isFetching} onRefresh={() => { void connections.refetch(); void permissions.refetch(); void plans.refetch(); void capabilityReality.refetch(); }} />}>
        ...
        {connection ? <>
          ...
          {tab === '可用能力' ? <CapabilityList connectionId={id} capabilities={capabilityReality.data?.capabilities ?? []} /> : null}
...
function CapabilityList({ connectionId, capabilities }: { connectionId: string; capabilities: CapabilityReality[] }) {
  return <><SectionTitle title="可用能力" /><View style={styles.card}>{capabilities.length > 0 ? capabilities.map((capability, index) => <Pressable accessibilityRole="button" onPress={() => router.push(`/connections/${connectionId}/capabilities/${capability.key}` as never)} key={capability.key} style={[styles.capabilityRealityRow, index < capabilities.length - 1 && styles.divider]}><View style={styles.rowIcon}><Ionicons name={capabilityIcon(capability.key)} size={18} color={colors.primary} /></View><View style={styles.rowCopy}><View style={styles.capabilityHeading}><Text style={styles.rowTitle}>{capability.name}</Text><Text style={[styles.badge, !capability.usable && styles.badgeBlocked]}>{capability.usable ? '当前可用' : '暂不可用'}</Text></View><View style={styles.dimensionChips}><DimensionChip label="官方" value={capabilityOfficialLabel(capability.providerAvailability)} ok={capability.providerAvailability === 'AVAILABLE'} /><DimensionChip label="实现" value={capabilityImplementationLabel(capability.implementation)} ok={capability.implementation === 'PRODUCTION' || capability.implementation === 'BETA'} /><DimensionChip label="授权" value={capabilityGrantLabel(capability.grant)} ok={capability.grant === 'GRANTED'} /><DimensionChip label="健康" value={capabilityHealthLabel(capability.health)} ok={capability.health === 'HEALTHY'} /></View>{capability.reasons.length > 0 ? <Text style={styles.capabilityReason}>{reasonLabel(capability.reasons[0]!)}</Text> : null}</View></Pressable>) : <Text style={styles.cardEmpty}>这个来源暂未声明可用能力。</Text>}</View></>;
}
