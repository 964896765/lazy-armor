...
export default function TabsLayout() {
  ...
  return (
    ...
    </Tabs>{pathname !== '/create' ? <Pressable accessibilityRole="button" accessibilityLabel="用 AI 创建计划草稿" onPress={() => router.push('/create' as never)} style={[styles.aiEntry, { bottom: Math.max(insets.bottom + 12, 22) }]}><Ionicons name="sparkles" size={17} color="#FFFFFF" /><Text style={styles.aiText}>AI 规划</Text></Pressable> : null}</View>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1 },
  aiEntry: { position: 'absolute', right: 14, minHeight: 42, paddingHorizontal: 14, borderRadius: 22, backgroundColor: '#087A5A', flexDirection: 'row', alignItems: 'center', gap: 6, shadowColor: '#064E3B', shadowOpacity: 0.18, shadowRadius: 9, elevation: 5 },
