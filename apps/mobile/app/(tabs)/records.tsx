...
import { executionListState, executionStatusLabel } from '../../src/execution-presenter';
...
export default function Records() {
  ...
  return (
    <ScrollView style={local.page} contentContainerStyle={local.content} refreshControl={token ? <RefreshControl refreshing={executions.isFetching} onRefresh={() => executions.refetch()} /> : undefined}>
      ...
      {state === 'ready' && executions.data?.map((item) => <Pressable key={item.id} onPress={() => router.push(`/executions/${item.id}` as never)}><View style={styles.card}><View style={local.row}><Text style={local.time}>{new Date(item.createdAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}</Text><Text style={local.status}>{executionStatusLabel(item.status)}</Text></View><Text style={styles.cardTitle}>{item.planName}</Text>{item.resultSummary && <Text style={styles.cardText}>{item.resultSummary}</Text>}</View></Pressable>)}
    </ScrollView>
  );
}

const local = StyleSheet.create({ page: { flex: 1, backgroundColor: '#F5F4EF' }, content: { padding: 24, paddingTop: 52 }, row: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }, time: { color: '#69756F' }, status: { color: '#287052', fontWeight: '700' } });
