...
import { connectionRecoveryAction, connectionStatusLabel } from '../../src/connection-presenter';
...
export default function Today() {
  ...
  const count = (today.data?.pendingApprovals.length ?? 0) + (today.data?.connectionIssues.length ?? 0) + (today.data?.alerts.length ?? 0) + (today.data?.processed.length ?? 0);
  ...
  return <ScrollView style={local.page} contentContainerStyle={local.content} refreshControl={token ? <RefreshControl refreshing={today.isFetching} onRefresh={() => today.refetch()} /> : undefined}>
    ...
    {today.data?.connectionIssues.map((item) => <View style={[styles.card, local.connectionIssue]} key={`${item.planId}:${item.connectionId}`}><Text style={local.section}>需要处理 · 连接</Text><Text style={styles.cardTitle}>{item.providerName}{item.connectionStatus === 'reauthorization_required' ? '需要重新连接' : `：${connectionStatusLabel(item.connectionStatus)}`}</Text><Text style={styles.cardText}>“{item.planName}”仍会保留，其他可用来源会继续工作。</Text><Button title={connectionRecoveryAction(item.connectionStatus) ?? '查看连接'} onPress={() => router.push('/connections')} /></View>)}
