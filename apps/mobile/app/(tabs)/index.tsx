    ...
    <SafeAreaView edges={[]} style={styles.safeArea}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content} refreshControl={token ? <RefreshControl tintColor={colors.primary} refreshing={today.isFetching} onRefresh={() => today.refetch()} /> : undefined}>
        <View style={styles.sectionHeading}>
          ...
          <Pressable accessibilityRole="button" onPress={() => router.push('/plan-center' as never)} style={({ pressed }) => [styles.textAction, pressed && styles.pressed]}>
...
function DomainTreeNode({ domain, expanded, last, onToggle }: { domain: HomeDomainNode; expanded: boolean; last: boolean; onToggle: () => void }) {
  return <View style={!last && styles.domainDivider}>
    <Pressable accessibilityRole="button" accessibilityState={{ expanded }} onPress={onToggle} style={({ pressed }) => [styles.domainRow, pressed && styles.rowPressed]}><Ionicons name={expanded ? 'chevron-down' : 'chevron-forward'} size={18} color={colors.primary} /><Text style={styles.domainName}>{domain.label}</Text><Text style={styles.domainCount}>{domain.scenarios.length} 个场景</Text></Pressable>
    {expanded ? <View style={styles.scenarioList}>{domain.scenarios.map((scenario) => <Pressable key={`${scenario.productDomain}.${scenario.key}`} accessibilityRole="button" onPress={() => router.push(`/domains/${scenario.productDomain}/${scenario.key}` as never)} style={({ pressed }) => [styles.scenarioRow, pressed && styles.rowPressed]}><View style={styles.branch} /><Text style={styles.scenarioName}>{scenario.label}</Text><Text style={styles.scenarioBoundary}>查看真实状态</Text><Ionicons name="chevron-forward" size={16} color={colors.textMuted} /></Pressable>)}</View> : null}
  </View>;
}
...
const styles = StyleSheet.create({
  ...
  tree: { overflow: 'hidden', borderRadius: radius.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, domainDivider: { borderBottomWidth: 1, borderBottomColor: colors.border }, domainRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingHorizontal: spacing.md }, domainName: { ...typography.bodyStrong, color: colors.text, flex: 1 }, domainCount: { ...typography.caption, color: colors.textMuted }, scenarioList: { paddingBottom: spacing.xs, backgroundColor: '#FCFBF8' }, scenarioRow: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingLeft: 38, paddingRight: spacing.md }, branch: { width: 8, height: 1, backgroundColor: '#CFCBC2' }, scenarioName: { ...typography.body, color: colors.text, flex: 1 }, scenarioBoundary: { fontSize: 9, lineHeight: 13, color: colors.textMuted }, rowPressed: { backgroundColor: colors.pressed }, catalogBoundary: { fontSize: 10, lineHeight: 15, color: colors.textMuted, marginTop: spacing.sm },
