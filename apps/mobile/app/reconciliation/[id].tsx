...
export default function ReconciliationDetailPage() {
  ...
  return <RuntimeDetailScreen title="结果回查" subtitle="确认真实结果，而不是重复原动作" onBack={() => router.back()}>
    ...
    {data ? <>
      ...
      <RuntimeSection title="验证证据"><RuntimeCard>{data.evidence.length === 0 ? <RuntimeText>尚无回查证据，结果保持 {runtimeResultCopy(data.resultState)}。</RuntimeText> : data.evidence.map((item) => <RuntimeCard key={item.id} title={item.method}><RuntimeKeyValue label="结果" value={runtimeResultCopy(item.resultState)} /><RuntimeKeyValue label="证据来源" value={item.evidenceKey} /><RuntimeKeyValue label="证据指纹" value={shortHash(item.evidenceHash)} /><RuntimeKeyValue label="验证时间" value={displayTime(item.verifiedAt)} last /></RuntimeCard>)}</RuntimeCard></RuntimeSection>
      <RuntimeSection title="关联记录"><RuntimeCard><RuntimeKeyValue label="执行记录" value={data.executionId} /><RuntimeKeyValue label="操作" value={data.operationId} /><RuntimeKeyValue label="最后更新" value={displayTime(data.updatedAt)} last /><Pressable accessibilityRole="button" onPress={() => router.push(`/executions/${data.executionId}` as never)} style={styles.link}><Text style={styles.linkText}>查看执行记录</Text></Pressable></RuntimeCard></RuntimeSection>
    </> : null}
  </RuntimeDetailScreen>;
}
