...
interface ScenarioRuntimeEvidence {
  ...
  capabilities?: Array<{ capabilityKey: string; providerKey: string; connectionId: string; usable: boolean; dimensions: { declared: boolean; implemented: boolean; authorized: boolean; healthy: boolean } }>;
    ...
    {token && data && evidence && readiness.data ? <>
      ...
      <RuntimeSection title="所需事实"><RuntimeCard>
        ...
        {data.requiredFacts.map((key) => {
          ...
          return <View key={key} style={styles.listRow}><View style={styles.factIcon}><Ionicons name="document-text-outline" size={18} color={colors.primary} /></View><View style={styles.listCopy}><Text selectable style={styles.listTitle}>{key}</Text><Text style={styles.listMeta}>{available ? '有符合场景时效要求的事实类型' : '需要补充或更新信息'}</Text></View><Text style={[styles.factBadge, !available && styles.factMissing]}>{available ? '已获取' : '待补充'}</Text></View>;
        })}
      </RuntimeCard></RuntimeSection>
      <RuntimeSection title="来源与授权"><RuntimeCard>
        {(evidence.capabilities ?? []).length === 0 ? <RuntimeText>尚无匹配的连接能力。你可以前往连接中心查看信息来源。</RuntimeText> : evidence.capabilities?.map((capability) => <View key={`${capability.connectionId}:${capability.capabilityKey}`} style={styles.sourceCard}>
          <Text style={styles.listTitle}>{capability.providerKey}</Text><Text style={styles.listMeta}>{capability.capabilityKey}</Text>
          <View style={styles.dimensionRow}>{([{ key: 'declared', label: '官方开放' }, { key: 'implemented', label: '已实现' }, { key: 'authorized', label: '已授权' }, { key: 'healthy', label: '当前健康' }] as const).map((dimension) => <View key={dimension.key} style={styles.dimension}><Ionicons name={capability.dimensions[dimension.key] ? 'checkmark-circle' : 'remove-circle-outline'} size={16} color={capability.dimensions[dimension.key] ? colors.success : colors.textMuted} /><Text style={styles.listMeta}>{dimension.label}</Text></View>)}</View>
        </View>)}
        <Pressable accessibilityRole="button" onPress={() => router.push('/(tabs)/connections' as never)} style={styles.linkRow}><Text style={styles.linkText}>管理信息来源</Text><Ionicons name="arrow-forward" size={16} color={colors.primary} /></Pressable>
      </RuntimeCard></RuntimeSection>
      ...
      <RuntimeSection title="推荐计划"><RuntimeCard>{templates.isLoading ? <RuntimeText>正在读取计划方案…</RuntimeText> : templates.isError ? <RuntimeText>计划方案暂时无法读取。</RuntimeText> : matchingTemplates.length === 0 ? <RuntimeText>暂时没有适合这件事的计划方案。</RuntimeText> : matchingTemplates.map((template) => <Pressable accessibilityRole="button" key={template.key} onPress={() => router.push(`/templates/${template.key}` as never)} style={styles.listRow}><View style={styles.listCopy}><Text style={styles.listTitle}>{template.name}</Text><Text style={styles.listMeta}>{template.description}</Text></View><Ionicons name="chevron-forward" size={16} color={colors.textMuted} /></Pressable>)}</RuntimeCard></RuntimeSection>
