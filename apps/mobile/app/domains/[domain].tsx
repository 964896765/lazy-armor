  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView style={styles.page} contentContainerStyle={styles.content}>
        <WorkspaceHeader title={definition.label} subtitle={`${groupLabel(definition.group)} · 计划、资料与最近动态`} onBack={() => router.back()} />
  ...
  return (
    <>
      <View style={styles.statGrid}>
        <View style={styles.statCard}><Text style={styles.statValue}>{activePlans}</Text><Text style={styles.statLabel}>正在帮你</Text></View>
        <View style={styles.statCard}><Text style={styles.statValue}>{planCount}</Text><Text style={styles.statLabel}>已设置计划</Text></View>
      </View>
      <Text style={styles.sectionTitle}>最近动态</Text>
      <View style={styles.sectionBody}>{latest ? <><Text style={styles.activityTitle}>{latest.resultSummary ?? planStatusLabel(latest.status)}</Text><Text style={styles.activityDetail}>最近一次运行结果已收进记录。</Text></> : <Text style={styles.quietText}>该领域还没有运行记录。你可以先添加一项计划，或连接一个可用的信息来源。</Text>}</View>
      <Text style={styles.sectionTitle}>下一步</Text>
      <View style={styles.sectionBody}><Text style={styles.activityTitle}>从一件小事开始</Text><Text style={styles.activityDetail}>选择一个场景后，会明确告诉你看什么、何时判断、做到哪一步，以及是否需要确认。</Text><View style={styles.action}><Pressable accessibilityRole="button" onPress={() => router.push('/create' as never)}><Text style={styles.actionText}>添加计划</Text></Pressable></View></View>
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  statCard: { flex: 1, paddingVertical: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.border },
