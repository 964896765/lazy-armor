...
export default function TruthStorePage() {
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top']}><ScrollView style={styles.page} contentContainerStyle={styles.content}>
    ...
    {!token ? <Surface><EmptyState icon="▣" title="请先登录" description="登录后可以查看自己的已验证事实。" action={{ label: '去登录', onPress: () => router.push('/auth/login' as never) }} /></Surface> : null}
    {token && facts.isLoading ? <ActivityIndicator color={colors.primary} /> : null}
    {token && facts.isError ? <Surface><EmptyState icon="!" title="暂时无法读取事实" description="没有显示任何示例数据，也没有修改你的记录。" action={{ label: '返回通知来源', onPress: () => router.replace('/connections/notification-sources' as never) }} /></Surface> : null}
    {token && !facts.isLoading && !facts.isError && (facts.data?.length ?? 0) === 0 ? <Surface><EmptyState icon="▣" title="还没有已验证事实" description="先为已添加应用单独启用通知来源；收到的线索需要由你确认后才会显示在这里。" action={{ label: '管理通知来源', onPress: () => router.push('/connections/notification-sources' as never) }} /></Surface> : null}
