...
export default function TruthDetailPage() {
  ...
  return <RuntimeDetailScreen title="为什么相信这条信息" subtitle="查看真实来源、确认方式和记录时间" onBack={() => router.back()}>
    ...
    {fact ? <>
      ...
      <RuntimeSection title="信息从哪里来"><RuntimeCard>{fact.provenance.length === 0 ? <RuntimeText>尚无可安全展示的来源链；不会把缺失来源补成已验证。</RuntimeText> : fact.provenance.map((item) => <View key={`${item.truthRecordVersionId}-${item.evidenceHash}`} style={styles.source}><Ionicons name="layers-outline" size={19} color={colors.primary} /><View style={styles.sourceCopy}><Text style={styles.sourceTitle}>{item.providerKey}</Text><Text style={styles.sourceMeta}>{item.sourceMode} · {displayTime(item.observedAt)}</Text></View></View>)}</RuntimeCard></RuntimeSection>
      <RuntimeSection title="确认记录"><RuntimeCard>{presentations.isLoading ? <RuntimeText>正在读取已验证的事实摘要…</RuntimeText> : presentations.isError ? <RuntimeText>事实摘要暂时无法读取。</RuntimeText> : presentation ? <><RuntimeKeyValue label="事实" value={presentation.valueSummary || presentation.factKey} /><RuntimeKeyValue label="来源" value={presentation.sourceLabel || '来源未标注'} /><RuntimeKeyValue label="观察时间" value={presentation.observedAt ? displayTime(presentation.observedAt) : '时间未记录'} /></> : <RuntimeText>这条记录目前不在已验证事实列表中；不会将历史记录当作当前事实。</RuntimeText>}<RuntimeKeyValue label="资源类型" value={fact.resourceKey} /><RuntimeKeyValue label="主体" value={fact.subjectKey} /><RuntimeKeyValue label="确认方式" value={provenanceMethodLabel(fact.verifiedBy)} /><RuntimeKeyValue label="确认时间" value={displayTime(fact.verifiedAt)} last /></RuntimeCard></RuntimeSection>
...
const styles = StyleSheet.create({
  ...
  source: { minHeight: 58, flexDirection: 'row', alignItems: 'center', gap: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.border }, sourceCopy: { flex: 1 }, sourceTitle: { ...typography.bodyStrong, color: colors.text }, sourceMeta: { ...typography.caption, color: colors.textSecondary, marginTop: 3 },
