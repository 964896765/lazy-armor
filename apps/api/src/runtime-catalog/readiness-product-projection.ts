...
export function projectConsumerReadiness(input: Input): ConsumerReadinessProjection {
  ...
  else if (input.deviceRequired && !input.deviceOnline) userReadiness = 'DEVICE_OFFLINE';
  else if (grants.some((item) => item.grant === 'REVOKED' || item.grant === 'EXPIRED' || item.health === 'REAUTHORIZATION_REQUIRED' || item.health === 'PERMISSION_REVOKED')) userReadiness = 'NEEDS_PERMISSION';
  ...
  const copy: Record<ConsumerReadinessState, [string, string, string, ConsumerReadinessProjection['actionPath']]> = {
    ...
    NEEDS_PERMISSION: ['等待授权', '连接的权限已过期或被撤销。', '重新授权', '/connections'],
    NEEDS_DATA: ['等待真实数据', '尚未收到所需的近期可信事实。', '检查数据来源', '/connections'],
    DEVICE_OFFLINE: ['手机离线', '这项能力需要手机在线并保持设备心跳。', '检查手机连接', '/connections'],
    SERVICE_UNAVAILABLE: ['暂不可用', '平台能力尚未完成官方核实或实现，不能假装已可自动运行。', '查看可用连接', '/connections'],
