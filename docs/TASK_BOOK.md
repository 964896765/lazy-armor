...
| 通知来源 | 用户可对任意已添加 App 单独选择是否允许其通知成为来源。来源事件经过 Generic Parser / Classifier / Normalizer 后，才可提供给计划引擎。 | `if packageName == 某品牌` 作为核心解析分支；默认读取所有通知；用 Accessibility 泛化为任意 App 自动操作。 |
