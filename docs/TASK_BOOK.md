...
| 可信设备 | 创建或启用数据来源的 Generic Connection 必须绑定当前账号、当前安装、未撤销的密钥证明设备。私钥必须保留在 Android Keystore，服务端只接受一次性挑战的签名、公钥与指纹；挑战成功后签发短期设备会话，真实发现证据、连接创建和通知收据均须由同一密钥对方法、路径、正文哈希、时间戳和一次性请求 ID 签名。 | 把可复制安装 ID 当作高信任凭据；收集硬件序列号；未验证设备上传来源事件；重放短期会话内的请求 ID；将当前 `key_proven` 表述为硬件级证明。 |
...
| Truth Store | 事实必须使用品牌中立 `resourceKey`、来源收据绑定、验证方法、证据哈希、当前版本引用和不可变 `valueHash` 保存；事实记录、首个版本与 `currentVersionId` 必须在同一数据库事务内同生同灭。Plan Engine 只能读取 `verified` 且存在当前版本的事实。 | 在计划上下文中读取通知原文、未确认候选、Provider 名称、包名或 Adapter 专属字段；将多步事实写入拆成可留下半记录的独立提交；覆盖历史 PlanVersion/DefinitionHash。 |
...
| 迁移真实执行 | 多条顶层 DDL 的 Drizzle 迁移必须同时在 Journal 启用断点，并以 `--> statement-breakpoint` 分段；CI 在 MySQL 执行前必须调用真实 `readMigrationFiles` 验证分段输入。 | 把多个顶层 `CREATE`/`ALTER` 作为单条 MySQL SQL 发送；用静态 SQL 外观检查替代 Drizzle 实际读取或 MySQL 执行证据。 |
...
| 可信设备撤销 | 撤销设备后，所有绑定的 Generic Connection、短期设备会话与通知读取必须立即停用，后续来源收据必须失败关闭并记录审计。 |
