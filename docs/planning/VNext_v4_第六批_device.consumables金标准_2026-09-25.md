...
- 复用 `FactDemandResolver` + Reality Pipeline：`device.consumable.remaining_days` 事实只有取得可验证证据后才 `SATISFIED`；无数据时 `NEEDS_SOURCE`，绝不虚构设备实时读取或推断剩余寿命。
...
| FactDemand 无数据 → `NEEDS_SOURCE`（不猜寿命） | DETERMINISTIC_VERIFIED | — | — |
| FactDemand 取得证据 → `SATISFIED` + 跨用户隔离 | DETERMINISTIC_VERIFIED | — | — |
| 周期提醒 / 阈值提醒 / 购物清单（执行生命周期） | DETERMINISTIC_VERIFIED（既有 p1-device / p4-consumer-journeys） | — | — |
...
- **FactDemand / SourceResolver**：同时支持 `daily_life.delivery`（设备 App 通知源）与 `device.consumables`（用户登记 / edge-device 任务源），复用同一 `FactDemandResolverService` 与 Reality Pipeline，无需复制。
...
- 无数据时不推断剩余寿命，返回 `NEEDS_SOURCE` 并引导手动登记（`ALLOW_MANUAL_ASSISTED`）。
